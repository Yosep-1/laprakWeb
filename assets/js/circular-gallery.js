...
class Media {
  ...
  update(scroll, direction) {
    ...
    if (this.bend === 0) {
      ...
      if (this.bend > 0) {
        this.plane.position.y = -arc; this.plane.rotation.z = -Math.sign(x) * Math.asin(effectiveX / R);
      } else {
        this.plane.position.y = arc; this.plane.rotation.z = Math.sign(x) * Math.asin(effectiveX / R);
      }
    }
...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Plus Jakarta Sans', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'Prak. Modul 1' }
    ];
  ...
  destroy() { window.cancelAnimationFrame(this.raf); }
}

export function initCircularGallery(selector, options) {
